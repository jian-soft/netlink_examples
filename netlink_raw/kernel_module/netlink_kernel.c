...
static void netlink_recv_msg(struct sk_buff *skb) 
{
    ...
    printk(KERN_INFO "netlink_kernel: Send %s\n", msg);
    
    res = nlmsg_unicast(g_nl_sock, skb_out, pid);
    if (res < 0)
      printk(KERN_INFO "netlink_kernel: Error while sending skb to user\n");
}
...
static int __init netlink_kernel_init(void)
{
    ...
    printk(KERN_INFO "netlink_kernel: init module\n");
...
{
    printk(KERN_INFO "netlink_kernel: exit module\n");
